...
func (h *_default) LoginStudentAuth(c *gin.Context) {
	...
	case http.StatusOK:
		...
		c.JSON(status, gin.H{"status": status, "code": _code, "message": msg, "access_token": jwtToken, "student_uuid": rpcResp.LoggedInStudentUUID})
		entry.WithFields(logrus.Fields{"status": status, "code": _code, "message": msg, "access_token": jwtToken, "student_uuid": rpcResp.LoggedInStudentUUID}).Info()
		topSpan.LogFields(log.Int("status", status), log.Int("code", _code), log.String("message", msg),
